...
6. `수동매매`에서 매수금액(원)/매도수량(주) 입력 후 매수/매도.
   `평단 대비 자동 손절`을 켜고 하락률(%)을 저장하면 정규장 현재가가 계좌 평단의
...
키움 조회에 성공한 결과는 날짜·조회 조건별로 서버에 저장한다. 자정 이후 KRX 자료가
아직 없으면 저장된 결과와 조회 시각을 표시하고, KRX 자료가 게시되면 재조회 시 우선 사용한다.
저장된 키움 시세는 확정 종가가 아니며 장중 조회 결과일 수 있다. 이전에 저장한 결과가
없는 날짜는 기존처럼 자료 게시 대기를 안내한다. 저장 파일은 `backend/state.screener.*.json`이다.

실전 키움 계좌와 KRX 소스를 사용하는 서버는 평일 한국시간 15:35 이후 기본 조건(29~30%)을
자동 조회·저장한다. 60초 간격으로 확인하며 실패 시 재시도한다. 마감 후 자정 전에 서버를
켜도 수집하고, 이미 15:35 이후 저장된 결과가 있으면 건너뛴다. 서버가 꺼져 있으면 수집할 수
없으며 공휴일을 별도로 판별하지 않으므로, 이 결과도 KRX 확정 종가가 아닌 키움 스냅샷이다.

헤더의 **매매 / 상한가 종목** 탭으로 오간다(해시 라우팅 — 새로고침·뒤로가기 유지).

지정일 **D**의 종가가 **직전 거래일 D-1** 종가 대비 **+29%~+30%** 인 종목을
표(종목코드·종목명·시장·시가총액·거래량·종가·등락률)로 보여준다. 오늘 조회는 실전
키움 계좌의 `ka10017` 당일 시세를 사용하고, 과거 날짜는 KRX 확정 일별 데이터를
사용한다. D-1은 달력상 전날이 아니라 직전 *거래일*로, 주말·공휴일은 시세가
비어 오는 것으로 판별해 건너뛴다.
...
키움 REST에는 **전종목 과거 일별시세** 엔드포인트가 없다(종목별 조회뿐이라 날짜당
~2,800회 호출). 따라서 오늘은 당일 순위 API `ka10017`, 과거는 KRX Open API를
사용한다. 당일 시가총액은 직전 KRX 데이터의 상장주식수에 현재가를 곱해 계산한다.
...
KRX 전일 자료는 **영업일 기준 다음 날 오전 8시(KST)**에 갱신된다.
최근 날짜의 빈 자료는 `KRX 자료 게시 대기`로 안내하며, 오전 8시 직전의 빈 캐시는
게시 경계에서 만료한다. 게시 지연이나 한 시장만 게시된 응답은 30초 후 재조회한다.
`최근 거래일` 조회가 이전 자료로 내려간 경우 실제 표시 날짜와 안내를 함께 표시한다.
공휴일 달력은 별도로 관리하지 않으므로 휴장·게시 지연 여부는 데이터와 안내를 함께 확인한다.
...
접근토큰은 계좌별로 만료 시각을 보관하고, 만료 60초 전부터 갱신한다.
동시 조회의 발급 요청은 합치며, 인증 거부(8005/HTTP 401)를 받은 조회는 새 토큰으로
한 번만 재시도한다. 발급 실패 시 30초 간격으로 다시 시도하며 무효·만료 토큰은 재사용하지 않는다.
시작 시 인증 서버에 연결할 수 없어도 앱을 열어 실패 상태를 확인할 수 있다.

실시간 연결은 갱신 토큰으로 다시 로그인하고 잔고·종목명·초기 시세를 동기화한다.
계좌 카드에는 인증, 조회, 시세 연결, 최근 체결 수신 시각을 각각 표시한다.
상단 `앱 연결됨`은 브라우저와 Bach 서버 사이의 연결만 의미한다.
`backend/logs/events.log`에는 계좌별 인증 갱신·거부·실패와 만료 시각을 남긴다.
토큰 및 자격증명은 로그나 브라우저 상태에 포함하지 않는다.

주문 POST는 전송 계층에서 자동 재전송하지 않는다. 인증 복구의 조회 재시도도
주문에는 적용하지 않는다. 통신 오류로 접수 여부가 불명확하면 계좌 접수 내역을 확인해야 한다.

## 구조

차트에는 이 기능 적용 후 서버가 수신한 실제 체결을 표시한다. 매수는 빨간 B와
상향 화살표, 매도는 파란 S와 하향 화살표이며 같은 봉의 동일 방향 체결은
수량과 가중평균 체결가로 합쳐 표시한다. 일봉 전환 시에는 날짜별로 모인다.
키움이 제공하는 체결시각을 사용하고, 시각이 없는 통지는 수신 시각을 사용한다.
계좌별 `state.*.trades.json` 파일에 저장해 서버 재시작 후에도 유지한다.
서버가 꺼져 있거나 연결이 끊긴 동안의 체결 및 도입 이전 내역은 자동 복원하지 않는다.
차트 체결 표시는 약 5초 간격으로 갱신한다.
체결 처리에서는 기록을 큐에 넣고, 전용 작업 스레드가 중복 제거와 파일 저장을 수행한다.
차트 조회는 종목별 메모리 스냅샷만 읽으며 매매 경로는 조회·파일 저장을 기다리지 않는다.
정상 종료 시 대기 기록을 저장하되 최대 5초까지만 기다린다. 강제 종료 시 미저장 기록은 유실될 수 있다.
